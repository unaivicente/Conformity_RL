...
data_total$Group <- factor(data_total$Group, levels = c('C','I'))
...
total_block <- lmer(Inter_div_abs~Group*Block + (1|ID/AbsSuj),data=data_total)
total_block

phm <- pairs(lsmeans(total_block, ~Group*Block))
...
f1 <- ggplot(efdata1, aes(x=TrialRep, y=fit, color=Group, group=Group)) + 
  ...
  geom_ribbon(aes(ymin=fit-se, ymax=fit+se), linetype=2, alpha=0.1) + 
  labs(x= "Trial Repetition", y="Difference between participants", color="Groups", fill="Groups") + theme_gray() + theme(text=element_text(size=20))+
  theme_light()+
...
ggsave('difanswers.pdf', f1 , device = cairo_pdf, width = 25, height = 20, units="cm")

#PLOT EFFECTS OF 2ND INTERACTION


ef.b <- effect(term = "Group:Block", mod = total_block)
...
blockp
ggsave('difanswers_groupblock.pdf', blockp , device = cairo_pdf, width = 25, height = 20, units="cm")
...
p <-ggplot(dat, aes(x=Productivity_abs))+
  geom_histogram(aes(fill=Group),alpha=0.4,position= position_dodge(),bins = 40)+
...
ggsave('accuracy.pdf', p , device = cairo_pdf, width = 25, height = 20, units="cm")
...
ggsave('quest.pdf', pall , device = cairo_pdf, width = 60, height = 30, units="cm")
