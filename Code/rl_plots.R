...
dat1 <- read.csv('first_expdata2.csv', sep=',', header=TRUE)
...
p1 <- ggplot(dat1, aes(x=Group, y=Likelihood, color=Group)) + 
  geom_boxplot(notch=TRUE, width=0.5, fill=pltt, alpha=0.25) +
  ...
  theme_light() +
  ggtitle("A. Boxplot of Model fit by Group")
...
ggsave("rl_diags2.pdf",a3, device="pdf", width = 30, height = 15, units="cm")
